...
object RecurringDecimals extends App {
    ...
    def f2d(n: Long, d: Long): String = {
      ...
      def fractionDecimalsTailrec(num: Long, denom: Long, digits: List[Long], remainders: List[Long]): String = {
        ...
        def findRecurrenceStart(
          ...
          if (digits.isEmpty || remainders.isEmpty)
            -1
          else if (digit == digits.head && rem == remainders.head)
            currentIndex
          else
            findRecurrenceStart(digit, digits.tail, rem, remainders.tail, currentIndex + 1)
        }
      ...
      if (n > 0 && d < 0)
        ...
        if (remainder == 0)
          ...
          s"$quotient.${fractionDecimalsTailrec(remainder, d, List(), List())}"
      }
    }
  ...
  println(fractionToRecurringDecimals(1, 3)) // 0.(3)
  println(fractionToRecurringDecimals(1, 2)) // 0.5
  println(fractionToRecurringDecimals(4, 2)) // 2
  println(fractionToRecurringDecimals(1, 6)) // 0.1(6)
  println(fractionToRecurringDecimals(1, 333)) // 0.(003)
  println(fractionToRecurringDecimals(1, 7)) // 0.(142857)
  println(fractionToRecurringDecimals(-1, 2)) // -0.5
}
